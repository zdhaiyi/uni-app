						...
						androidVpnManager.onStatusUpdate((status) => {
							...
							if (status.connected) {
								...
								if (this.selectedNode !== null) {
									...
									this.connectionInfo =
										`${selectedNodeData.tag} · ${selectedNodeData.host}:${selectedNodeData.port}`
								}
						...
						success: (res) => {
							if (res.statusCode === 200 && res.data) {
								...
								const processedNodes = newNodes.map(node => ({
									...
									isAvailable: node.status === true && new Date(node
										.expire) > new Date(),
								...
								if (isLoadMore) {
									...
									// 自动选择第一个可用节点
									const firstAvailableIndex = processedNodes.findIndex(node => node
										.isAvailable)
